...
class SubprojectsActivity(QDialog, Ui_Activity_Wrap_Dialog):
    ...
    def exec_download(self):
        self.console.command_run(self._model.model_project().meson().subprojects().download([self.edit_wrap_item.text()]))

    @pyqtSlot()
    def exec_checkout(self):
        self.console.command_run(self._model.model_project().meson().subprojects().checkout([self.edit_wrap_item.text()]))

    @pyqtSlot()
    def exec_update(self):
        self.console.command_run(self._model.model_project().meson().subprojects().update([self.edit_wrap_item.text()]))
